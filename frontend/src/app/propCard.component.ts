...
@Component({
    ...
    <div *ngIf="propKeys.length == 0">
        <!-- This is a reference array -->
            <div *ngIf='isArray()'>
            <arrayInput [required] = 'isRequired'[name]='getNameRef()'  [ref]='this.ref' route={{this.parents}} [prop]='' [props]='props' index={{index}}>
            </arrayInput>
            </div>
            <div *ngIf='isObject()'>
        ...
        <div *ngIf='hasParents()'>
            <div  class="spacing" [ngSwitch]='getPropertyType(prop, props)' id="inline" style="display:inline">
                <div *ngSwitchCase="'array'">
                        <arrayInput [required] = 'isRequired'[name] ='getPropertyName(prop, props)' [ref]='getArrayRef(prop,props)' route={{this.parents}} [prop]='prop' [props]='props' index={{index}}>
                        </arrayInput>
                </div>
                <div *ngSwitchCase="'string'" style="display:inline" id="inline">
                        <p id="inline">{{getPropertyName(prop, props)}}: </p>
                        <stringInput [required] = 'isRequired'route={{getPath(prop)}} [prop]='getPropfromKeys(prop,propKeys)' [prop] = 'prop' [props] = 'props'>
                        </stringInput>
                </div>
                <div *ngSwitchCase="'boolean'" style="display:inline"> 
                        <p id="inline">{{getPropertyName(prop, props)}}: </p>
                        <boolInput [required] = 'isRequired'route={{getPath(prop)}} [props] = 'props'>
                        </boolInput>
                </div>
            </div>
        </div>
...
export class PropComponent implements OnInit {
    ...
    getPath(prop : any) {
        ...
        if(this.getPropertyType(prop,this.props) == 'array') return this.parents +`[${this.index}]`;
    ...
    ngOnInit(): void {
        ...
        this.isRequired = this.required.includes(this.getNameRef())
    }
    title = 'jsonTalkSoft';
}
